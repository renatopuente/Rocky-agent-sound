...
for (const soundFile of soundFiles) {
  ...
  if (!fs.existsSync(src)) {
        console.error(`No se encontro ${soundFile} en el paquete.`);
        process.exit(1);
  }
...
if (fs.existsSync(settingsPath)) {
    try {
          ...
          console.error('settings.json existente tiene JSON invalido. Creando backup...');
          fs.copyFileSync(settingsPath, settingsPath + '.bak');
    }
}

// 4. Construir entrada del hook con seleccion aleatoria entre Rocky_A y Rocky_B
const hookCommand =
...
console.log('  Settings en          : ' + settingsPath);
console.log('');
console.log('  El sonido (Rocky_A o Rocky_B aleatorio) se reproducira cuando Claude:');
